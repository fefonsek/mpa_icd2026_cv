...
# 4. Estimativa Monte Carlo
aprox_monte_carlo <- (b - a) * mean(f(u))
aprox_monte_carlo

# 5. Aproximação numérica de referência
integral <- integrate(f, a, b)
valor_integral <- integral$value
valor_integral

# 6. Erro absoluto
erro_absoluto <- abs(aprox_monte_carlo - valor_integral)
erro_absoluto


```

A estimativa de Monte Carlo é próxima do valor obtido por integrate().<br>
Diferenças ocorrem devido à variabilidade amostral do método Monte Carlo.<br>
Com aumento de N, espera-se redução do erro.<br>